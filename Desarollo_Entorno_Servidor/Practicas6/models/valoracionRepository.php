...
class ValoracionRepository{
    ...
    public static function setValoracion($idUser,$idArticle,$valoracion ){
        $entra = ValoracionRepository::comprobarSiHaValorado($idUser);
    ...
    public static function comprobarSiHaValorado($idUser){
        $db = Conectar::conexion();
        $q = "SELECT  valoracion FROM valoracion where id_user = '".$idUser."' ";
        $result = $db->query($q);
        $datos = $result->fetch_row();
        $valoracion;
        if(!$datos){
